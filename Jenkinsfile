pipeline{
  ...
  stages{
    ...
    stage("sonar report"){
      steps{
        script{
          publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, includes: 'target/sonar/*.html', 
                       keepAll: false, reportDir: 'target/sonar', reportFiles: 'index.html', reportName: 'Sonar Report', reportTitles: '', 
